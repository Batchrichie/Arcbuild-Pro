...
BEGIN
  ...
  -- Credit revenue (4600)
  INSERT INTO journal_lines (journal_entry_id, account_code, account_name, credit, project_id)
  VALUES (v_journal_id, '4600', (SELECT account_name FROM chart_of_accounts WHERE account_code = '4600'), v_period_rev, p_project_id);

  -- Debit either contract asset or advance billings
  IF v_contract_asset > 0 THEN
    INSERT INTO journal_lines (journal_entry_id, account_code, account_name, debit, project_id)
    VALUES (v_journal_id, '1400', (SELECT account_name FROM chart_of_accounts WHERE account_code = '1400'), v_period_rev, p_project_id);
  ELSIF v_advance_billing > 0 THEN
    INSERT INTO journal_lines (journal_entry_id, account_code, account_name, debit, project_id)
    VALUES (v_journal_id, '2300', (SELECT account_name FROM chart_of_accounts WHERE account_code = '2300'), v_period_rev, p_project_id);
  ELSE
  ...
  -- Mirror journal_lines into ledger_entries
  FOR rec IN SELECT * FROM journal_lines WHERE journal_entry_id = v_journal_id LOOP
    INSERT INTO ledger_entries (journal_entry_id, account_code, account_name, debit_amount, credit_amount, project_id, created_at)
    VALUES (v_journal_id, rec.account_code, rec.account_name, COALESCE(rec.debit,0), COALESCE(rec.credit,0), p_project_id, NOW());
  END LOOP;
